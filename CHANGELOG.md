...
- **Kiro CLI Support**: Added support for Kiro CLI agent
  - Added `kiro-cli` to `AGENT_CONFIG` with proper metadata (`.kiro/commands/` directory structure)
  - Updated CLI help text to include `kiro-cli` as an available option
  - Added Kiro CLI to release package generation scripts
  - Added Kiro CLI to agent context update scripts (both bash and PowerShell)
  - Updated documentation (README.md and AGENTS.md) to include Kiro CLI in supported agents list
